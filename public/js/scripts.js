            ...
            contents.forEach((content, index) => {
                ...
                if (next) {
                    next.addEventListener('click', () => {
                        ...
                        var tabFormId = content.querySelector('.property-create-form').id;
                        const submitdata = MaselasubmitProperty(tabFormId, tabBar, index + 1);
...
function initDropzonePropertyImages(imagelist) {
    var warnings = document.querySelector("#media-warning-list");
    warnings.innerHTML = "";
    if ($('#property-images').length) {
        ...
        if (typeof imagelist !== 'undefined') {
            $.each(imagelist, function (key, value) {
                ...
                let mockFile = {name: value, size: 1024};
                let image_url = "/media/property/" + value;
                property_images.displayExistingFile(mockFile, image_url);
            });
        }

        property_images.on("sending", function (file, xhr, formData) {
            formData.append("property_id", document.querySelector("#input_propety_id").value);
            formData.append("current_tab", 'sp-media-form');
            formData.append("latlong", document.querySelector("#map_lat_long").value);
        });
        ...
        property_images.on("removedfile", function (file) {
            ...
            $.ajax({
                ...
                url: '/property/create',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                data: {current_tab: 'sp-media-form-remove',name: name, property_id: document.querySelector("#input_propety_id").value},
...
//                    console.log('success: ' + data);
        ...
        property_images.on("success", function (file, responseText) {
